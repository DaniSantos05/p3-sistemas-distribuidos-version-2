...
# Compatibilidad con tirpc (Linux moderno) o nsl (sistemas clasicos)
RPC_INC =
RPC_LIBS = -lnsl
...
LIB         = libclaves.so
CLIENT      = app-cliente
CLIENT_CONC = app-cliente-concurrente
...
APP_CLIENT_CONC_O = $(SRCDIR)/app-cliente-concurrente.o
...
all: $(LIB) $(CLIENT) $(CLIENT_CONC) $(SERVER)

# ==================================================
# 1. GENERACIÓN RPC
# Generamos cada fichero por separado para no machacar
# clavesRPC_server.c, que es codigo nuestro
# ==================================================
...
$(APP_CLIENT_CONC_O): $(SRCDIR)/app-cliente-concurrente.c $(SRCDIR)/claves.h
	$(CC) $(CFLAGS) -c $< -o $@
...
$(CLIENT_CONC): $(APP_CLIENT_CONC_O) $(LIB)
	$(CC) $(LDFLAGS) -o $@ $(APP_CLIENT_CONC_O) -lclaves $(LDLIBS)
...
clean:
	rm -f $(SRCDIR)/*.o
	rm -f $(LIB) $(CLIENT) $(CLIENT_CONC) $(SERVER)
